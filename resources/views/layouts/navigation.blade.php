...
<nav x-data="nav()" @keydown.window="if($event.key === 'Escape'){ closeAll() }" class="bg-white border-b">
    ...
    <div class="flex items-center justify-between h-16">
      <div class="flex items-center nav-left-gap">
        <a href="{{ route('home') }}" class="flex items-center gap-3">
          <img src="{{ asset('img/igniscode.png') }}" alt="Logo" class="w-10 h-10 object-contain">
          <span class="font-extrabold text-lg leading-none">{{ config('app.name') }}</span>
        </a>

        {{-- Desktop primary navigation (visible to guests only) --}}
        @guest
        <div class="hidden md:flex md:items-center md:ml-6 md:space-x-3">
          ...
          <a href="{{ route('about') }}" class="nav-link inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 rounded hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-300">
             About
          </a>
        </div>
        @endguest
      ...
      <div class="flex items-center gap-4">
        ...
        @else
          <div class="hidden md:flex md:items-center md:gap-2">
            {{-- Dashboard --}}
            <a href="{{ route($dashboardRoute) }}" class="icon-nav-item" title="Dashboard">
  ...
  <div x-show="mobile" x-cloak x-transition class="md:hidden mobile-backdrop border-t">
    <div class="px-4 py-4 space-y-3">
      @guest
        <a @click="mobile = false" href="{{ route('home') }}" class="block py-2 text-gray-700">Home</a>
        <a @click="mobile = false" href="{{ route('blogs.index') }}" class="block py-2 text-gray-700">Blog</a>
        <a @click="mobile = false" href="{{ route('contact.show') }}" class="block py-2 text-gray-700">Contact</a>
        <a @click="mobile = false" href="{{ route('about') }}" class="block py-2 text-gray-700">About</a>
