    ...
    <div class="grid lg:grid-cols-2 gap-12 items-center">
      <div class="relative z-10" data-reveal>
        ...
        <div class="flex flex-wrap gap-4">
          <a href="{{ route('register') }}" class="inline-flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white rounded-full font-semibold shadow-xl shadow-indigo-200 hover:bg-indigo-700 transform hover:-translate-y-1 transition-all duration-300" aria-label="Create free account">
          ...
          <a href="{{ route('home') }}#explore" class="inline-flex items-center gap-2 px-8 py-4 border-2 border-slate-200 rounded-full font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-300" aria-label="Explore courses">
            Explore programs
          </a>
        </div>
  ...
  <section id="explore" class="grid gap-12 lg:grid-cols-3">
    <main class="lg:col-span-2 space-y-10">
      ...
      <div class="flex items-center justify-between border-b border-slate-100 pb-6">
        ...
        <a href="{{ route('blogs.index') ?? '#' }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-slate-100 text-slate-700 rounded-full font-bold text-sm hover:bg-indigo-600 hover:text-white transition-all duration-300 group" aria-label="View all posts">
          View all posts
    ...
    <aside class="space-y-8">
      ...
      <div class="relative group hidden lg:block">
          <form method="GET" action="{{ route('blogs.index') }}" role="search" aria-label="Search posts">
      ...
      <div class="p-6 bg-white rounded-[1.25rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        <div class="flex flex-wrap gap-2">
          @php $tags = ['laravel', 'react', 'tailwind', 'php', 'javascript', 'vue']; @endphp
          @foreach($tags as $t)
            <a href="{{ route('blogs.index', ['tag' => $t]) }}" 
      ...
      <div class="p-6 bg-white rounded-[1.25rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        <div class="space-y-6">
          @php $trainers = [['name' => 'Jane Doe', 'courses' => 12, 'color' => 'indigo'], ['name' => 'John Smith', 'courses' => 8, 'color' => 'emerald']]; @endphp
          @foreach($trainers as $trainer)
            <div class="flex items-center justify-between group cursor-pointer">
              <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-2xl bg-{{ $trainer['color'] }}-100 flex items-center justify-center text-{{ $trainer['color'] }}-600 font-bold text-lg group-hover:scale-110 transition-transform" aria-hidden="true">
                  {{ substr($trainer['name'], 0, 1) }}
                </div>
                <div>
                    <div class="font-semibold text-slate-900">{{ $trainer['name'] }}</div>
                    <div class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ $trainer['courses'] }} Programs</div>
                </div>
              </div>
              <svg class="w-5 h-5 text-slate-300 group-hover:text-{{ $trainer['color'] }}-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7" /></svg>
            </div>
          @endforeach
        </div>
    ...
    <div class="relative p-12 md:p-20 bg-slate-900 rounded-[3rem] overflow-hidden">
        ...
        <div class="relative z-10 text-center max-w-2xl mx-auto">
            ...
            <div class="flex flex-wrap justify-center gap-4">
                <a href="{{ route('register') }}" class="px-8 py-4 bg-white text-slate-900 rounded-full font-extrabold hover:bg-indigo-500 hover:text-white hover:-translate-y-1 transition-all duration-300 shadow-2xl">
                    Create free account
                </a>
                <a href="{{ route('contact.show') }}" class="px-8 py-4 border border-white/20 text-white rounded-full font-semibold hover:bg-white/10 transition-all duration-300">
                    Contact sales
                </a>
            </div>
        </div>
