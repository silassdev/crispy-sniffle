          ...
          @foreach($trainers as $trainer)
            <div class="flex items-center justify-between group cursor-pointer">
              <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-2xl bg-{{ $trainer['color'] }}-100 flex items-center justify-center text-{{ $trainer['color'] }}-600 font-black text-lg group-hover:scale-110 transition-transform">
                  {{ substr($trainer['name'], 0, 1) }}
                </div>
                <div>
                    <div class="font-bold text-slate-900">{{ $trainer['name'] }}</div>
                    <div class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ $trainer['courses'] }} Courses</div>
                </div>
              </div>
    ...
    <div class="relative p-12 md:p-20 bg-slate-900 rounded-[3rem] overflow-hidden">
        ...
        <div class="relative z-10 text-center max-w-2xl mx-auto">
            ...
            <div class="flex flex-wrap justify-center gap-4">
                <a href="{{ route('register') }}" class="px-8 py-4 bg-white text-slate-900 rounded-full font-black hover:bg-indigo-500 hover:text-white hover:-translate-y-1 transition-all duration-300 shadow-2xl">
                    Create free account
                </a>
                <a href="{{ route('contact.show') }}" class="px-8 py-4 border border-white/20 text-white rounded-full font-bold hover:bg-white/10 transition-all duration-300">
                    Contact sales
                </a>
            </div>
        </div>
