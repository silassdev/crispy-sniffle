        ...
        @foreach($items as $n)
          ...
          <div @class([
            ...
            <div class="flex flex-col items-center gap-2 mt-1">
              <input type="checkbox" wire:model.live="selected" value="{{ $n->id }}" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 shadow-sm transition-all" />
              @if(!$isRead)
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-3 mb-1">
                <div>
                  <h4 class="font-bold text-slate-900 truncate pr-6">{{ $title }}</h4>
                ...
                <div class="flex items-center gap-1">
                   <button wire:click="delete('{{ $n->id }}')" class="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100" title="Delete">
              ...
              <div class="mt-4 flex items-center gap-2">
                @if($link)
                  <button wire:click="markAndGo('{{ $n->id }}', '{{ $link }}')" class="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors shadow-sm">
                    View Details
                  </button>
                @endif
                
                @if(!$isRead)
                  <button wire:click="markAsRead('{{ $n->id }}')" class="px-3 py-1.5 text-indigo-600 hover:bg-indigo-50 border border-transparent hover:border-indigo-100 rounded-lg text-xs font-bold transition-all">
                    Mark Read
                  </button>
                @endif
              </div>
