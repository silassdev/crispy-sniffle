@props(['role' => 'student', 'bg' => 'bg-gray-700 text-white'])

<aside class="w-64 min-h-screen bg-white border-r" x-data="{ collapsed: false }">
  <div class="p-4">
    ...
    {{-- collapse toggle (mobile / narrow) --}}
    <div class="px-2 mb-2 sm:hidden">
      <button @click="collapsed = !collapsed" class="w-full text-left p-2 rounded border hover:bg-gray-50">
        <span x-text="collapsed ? 'Open menu' : 'Close menu'"></span>
      </button>
    </div>

    {{-- navigation --}}
    <nav class="space-y-1" :class="{ 'hidden': collapsed }" aria-label="Dashboard navigation">

      @php
        // shared rendering helper for an item
        $renderItem = function($section, $label, $svg) use ($role) {
            return '
            <button
              wire:click="showSection(\''.$section.'\')"
              type="button"
              class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3"
              :class="$activeSection === \''.$section.'\' ? \'bg-gray-100\' : \'\'"
              aria-current="{{ $activeSection === \''.$section.'\' ? \'page\' : false }}"
              data-section="'.$section.'"
            >
              <span class="inline-block">'.$svg.'</span>
      ...
      @if($role === 'admin')
        {{-- Students --}}
        <button
          wire:click="showSection('students')"
          type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3"
          :aria-current="$wire.activeSection === 'students' ? 'page' : false"
        >
        ...
        {{-- Admins --}}
        <button
          wire:click="showSection('admins')"
          type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3"
          :aria-current="$wire.activeSection === 'admins' ? 'page' : false"
        >
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        ...
        {{-- Trainers (example showing JS-emitted event support) --}}
        <button
          onclick="emitLivewire('showSection', 'trainers')"
          type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3"
        ...
        {{-- Community --}}
        <button wire:click="showSection('community')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        ...
        {{-- Comments --}}
        <button wire:click="showSection('comments')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        ...
        {{-- Posts --}}
        <button wire:click="showSection('posts')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        ...
        {{-- Feedback --}}
        <button wire:click="showSection('feedback')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 15a4 4 0 01-4 4H8l-5 3V6a4 4 0 014-4h10a4 4 0 014 4v9z" />
        ...
        {{-- Other actions --}}
        <button wire:click="showSection('other-actions')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
      ...
      @elseif($role === 'trainer')
        <button wire:click="showSection('courses')" type="button"
          class="dash-item w-full text-left p-2 rounded hover:bg-gray-100 flex items-center gap-3">
          <span class="inline-block">
            <svg class="w-5 h-5 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
