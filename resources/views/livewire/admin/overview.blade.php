        ...
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-center gap-3 min-w-0">
            <div class="relative flex-shrink-0">
              <div class="w-12 h-12 rounded-xl bg-gradient-to-tr from-{{ $card['from'] }} to-{{ $card['to'] }} flex items-center justify-center shadow-sm">
                {{-- keep icon size small and centered --}}
              ...
              {{-- decorative SVG blob — positioned without negative offsets to avoid clipping --}}
              <svg class="absolute -right-3 -top-3 w-20 h-20 opacity-10 pointer-events-none" viewBox="0 0 100 100" fill="none" aria-hidden="true">
                <defs>
                  <linearGradient id="g{{ $loop->index }}" x1="0" x2="1">
                    <stop offset="0" stop-color="#000" stop-opacity="0.2"></stop>
                    <stop offset="1" stop-color="#000" stop-opacity="0.0"></stop>
                  </linearGradient>
                </defs>
                <circle cx="50" cy="50" r="40" fill="url(#g{{ $loop->index }})"/>
              </svg>
            </div>

            <div class="min-w-0">
              <div class="text-sm text-slate-500 dark:text-slate-300 truncate">{{ $card['label'] }}</div>
          ...
          <div class="flex flex-col items-end ml-2">
            ...
            <div class="text-xs text-slate-400 dark:text-slate-500">{{ $card['trendNote'] }}</div>
          </div>
        </div>
      </div>
      @endforeach
