        ...
        @forelse($courses as $c)
            <div class="group relative bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col">
                ...
                <div class="relative h-48 overflow-hidden bg-slate-100">
                    @if($c->hasMedia('illustration'))
                        <img src="{{ $c->getFirstMediaUrl('illustration') }}" alt="{{ $c->title }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    @else
                ...
                <div class="p-6 flex-1 flex flex-col">
                    <div class="flex-1">
                        <div class="text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">ID: {{ $c->course_id }}</div>
                        ...
                        <p class="text-slate-500 text-sm line-clamp-2 mb-4">
                            {{ $c->excerpt ?? 'No description provided.' }}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                        <div class="flex items-center text-slate-600 font-medium">
                            ...
                            <span class="text-sm">{{ $c->enrollments()->count() }} Students</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <a href="{{ route('trainer.courses.show', $c->id) }}" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all" title="View Details">
                            ...
                            <button wire:click="edit({{ $c->id }})" class="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all" title="Edit Course">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative w-full max-w-4xl mx-auto my-6 px-4 z-10 animate-in fade-in zoom-in duration-300">
                <div class="relative bg-white rounded-[2.5rem] shadow-2xl flex flex-col w-full outline-none focus:outline-none overflow-hidden">
                    ...
                    <div class="flex items-center justify-between p-8 border-b border-slate-100">
                        <div>
                            <h3 class="text-2xl font-bold text-slate-900">{{ $courseId ? 'Edit Course' : 'Create New Course' }}</h3>
                            <p class="text-sm text-slate-500 mt-1">Fill in the details below to {{ $courseId ? 'update your' : 'setup your new' }} course.</p>
                        </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input wire:model.defer="excerpt" type="text" placeholder="Brief summary for the course card" 
                                        class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all" />
                                    @error('excerpt') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('body') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative group cursor-pointer">
                                        <div class="w-full h-32 border-2 border-dashed border-slate-200 group-hover:border-indigo-500 rounded-2xl flex flex-col items-center justify-center transition-colors overflow-hidden relative">
                                            @if($illustration)
                                                <img src="{{ $illustration->temporaryUrl() }}" class="absolute inset-0 w-full h-full object-cover">
                                        ...
                                        @error('illustration') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                ...
                                <div>
                                    ...
                                    <div class="w-full min-h-[80px] border border-slate-200 rounded-2xl p-4">
                                        ...
                                        @error('attachments.*') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
